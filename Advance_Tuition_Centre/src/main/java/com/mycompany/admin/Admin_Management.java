...
public class Admin_Management {
    ...
    public static void Admin_Management(){
        base = new Base_Frame("Admin Management", 910, 500);
        
        Label_Frame title = new Label_Frame("Admin Management", 30, 30, 280, 35);
        title.font(true, 25);
    ...
    public static void display_data(){
        ...
        String[] data_file = file.read("Advance_Tuition_Centre/src/main/java/com/mycompany/data/admin.txt");
    ...
    private static void select(String selection){
        switch (selection) {
            ...
            case "update" : 
                ...
                String[][] admin_data = table.get_data(new int[] {0, 1, 2, 3});
                update_file.clear_and_update("Advance_Tuition_Centre/src/main/java/com/mycompany/data/admin.txt", admin_data);
                String[][] admin_account = table.get_data(new int[] {0, 4, 5});
                for (String[] row : admin_account){
            ...
            case "delete" :
                ...
                String admin_id = Message_Frame.input_frame("Remove Admin", "Please enter admin ID.");
                if (admin_id == null || admin_id.equals("")){
                    break;
                }

                boolean confirm = Message_Frame.confirm_frame("Confirm Deletion", String.format("Do you really want to delete this admin (%s)?", admin_id));
                if (confirm == true){
                    delete.delete_data("Advance_Tuition_Centre/src/main/java/com/mycompany/data/admin.txt", admin_id);
                    delete.delete_data("Advance_Tuition_Centre/src/main/java/com/mycompany/data/user.txt", admin_id);
                    Message_Frame.message_frame("Deletion Successful", "Successfully deleted admin.");
                }
                display_data();
                break;
        }
