...
public class Tutor_Management {
    static Base_Frame base;
    static Table_Frame table;
    static Read file = new Read();

    public static void Tutor_Management(){
        base = new Base_Frame("Tutor Management", 910, 500);
        
        Label_Frame title = new Label_Frame("Tutor Management", 30, 30, 280, 35);
        title.font(true, 25);
    ...
    public static void display_data(){
        String[] column = {"ID", "Name", "IC/Passport", "Email", "Contact Number", "Certificate", "Username", "Password"};
        String[] data_file = file.read("Advance_Tuition_Centre/src/main/java/com/mycompany/data/tutor.txt");
        ...
        for (int i = 0; i < data_file.length; i++){
            ...
            table_data[i][4] = data[4];
            table_data[i][5] = data[5];
            for (int j = 0; j < user_account.length; j++){
                user = user_account[j].split(";");
                if (data[0].equals(user[0])){
                    table_data[i][6] = user[1];
                    table_data[i][7] = user[2];
                    break;
                }
            }
        }
    ...
    private static void select(String selection){
        switch (selection) {
            ...
            case "update" : 
                ...
                String[][] tutor_data = table.get_data(new int[] {0, 1, 2, 3, 4, 5});
                update_file.clear_and_update("Advance_Tuition_Centre/src/main/java/com/mycompany/data/tutor.txt", tutor_data);
                String[][] tutor_account = table.get_data(new int[] {0, 6, 7});
                for (String[] row : tutor_account){
            ...
            case "delete" :
                ...
                String tutor_id = Message_Frame.input_frame("Remove Tutor", "Please enter tutor ID.");
                if (tutor_id == null || tutor_id.equals("")){
                    break;
                }

                boolean confirm = Message_Frame.confirm_frame("Confirm Deletion", String.format("Do you really want to delete this tutor (%s)?", tutor_id));
                if (confirm == true){
                    delete.delete_data("Advance_Tuition_Centre/src/main/java/com/mycompany/data/tutor.txt", tutor_id);
                    delete.delete_data("Advance_Tuition_Centre/src/main/java/com/mycompany/data/user.txt", tutor_id);
                    Message_Frame.message_frame("Deletion Successful", "Successfully deleted tutor.");
                }
                display_data();
                break;
        }
